...
Commontator.configure do |config|
  ...
  config.user_link_proc = lambda { |user, app_routes| user.path }
  ...
  config.thread_moderator_proc = lambda { |thread, user| user.role_id == 3 ||
                                          (user.role_id == 2 && thread.commontable.seller_id == user.id) }
